...
class GetIngredientsByTypeCommandHandlerTest extends TestCase
{
    public function test_handle_returns_an_array_of_std_objects_containing_ingredient_information_based_on_the_same_type()
    {
        ...
        $handler = new GetIngredientsByTypeCommandHandler($orchestrator->reveal(), $presenter);

        $command = new GetIngredientsByTypeCommand('Rose Wine');

        $orchestrator->getIngredientsByType($command->getType())->willReturn(new Collection([
